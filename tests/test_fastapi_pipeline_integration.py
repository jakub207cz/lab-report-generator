...
def test_generate_endpoint_uses_pipeline_and_returns_docx(monkeypatch) -> None:
    ...
    def fake_generate(api_key, model_name, topic, inputs_map, is_handwritten, report_scope):
        calls["generation"] += 1
        assert api_key == "test-key"
        assert topic == "Test tema"
        assert report_scope == "full"
...
def test_generate_endpoint_keeps_uploaded_waveforms_and_xlsx_charts_together(monkeypatch) -> None:
    calls = {"generation": 0}
    ...
    async def fake_extract_content(files, section_name, image_counter_start):
        ...
        if section_name == "waveforms":
            img = Image.new("RGB", (16, 16), color="green")
            asset = ImageAsset(image_id="IMG-321", filename="uzivatel-graf.png", section="waveforms")
            return (
                "Uživatelský graf",
                [img],
                [asset],
    ...
    def fake_generate(api_key, model_name, topic, inputs_map, is_handwritten, report_scope):
        ...
        assert report_scope == "full"
        assert inputs_map["data_images"] == []
        assert len(inputs_map["waveforms_images"]) == 2
        assert inputs_map["waveforms_image_ids"] == ["IMG-321", "IMG-123"]
        assert "[Graf" not in inputs_map["waveforms_text"]
        return LabReportData(
            teorie="Test teorie",
            postup="Test postup",
            priklad_vypoctu="Test vypocet",
